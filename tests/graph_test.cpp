...
TEST_F(GraphTest, ReadFromMetis) {
  Graph graph;
  graph.readMetis("test_metis.txt");
  ...
  EXPECT_EQ(graph.beginEdge(1), 2);
  EXPECT_EQ(graph.endEdge(1), 4);

  EXPECT_EQ(graph.beginEdge(2), 4);
  EXPECT_EQ(graph.endEdge(2), 6);

  EXPECT_EQ(graph.beginEdge(3), 6);
  EXPECT_EQ(graph.endEdge(3), 8);
}
